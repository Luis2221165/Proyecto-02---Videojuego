...
void loop() {
    ...
    if (joyHorizValue >= thresholdDown && joyHorizValue <= thresholdUp) {
        if (joyVertValue > thresholdUp && lastVertState != "y") {  // Cambié "w" por "y"
            sendData("y");
            lastVertState = "y";
        } 
        else if (joyVertValue < thresholdDown && lastVertState != "h") {  // Cambié "s" por "h"
            sendData("h");
            lastVertState = "h";
        } 
    ...
    if (joyVertValue >= thresholdDown && joyVertValue <= thresholdUp) {
        if (joyHorizValue > thresholdUp && lastHorizState != "g") {  // Cambié "a" por "g"
            sendData("g");
            lastHorizState = "g";
        } 
        else if (joyHorizValue < thresholdDown && lastHorizState != "j") {  // Cambié "d" por "j"
            sendData("j");
            lastHorizState = "j";
        } 
